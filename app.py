...
def calc_double_sine_gdd(t_min_today, t_max_today, t_min_tomorrow, T_base):
    ...
    # Use interpolated temperature curve from max to next min
    # t_max_to_min_avg = (t_max_today + t_min_tomorrow) / 2
    if t_max_today > t_min_tomorrow:
        # Cooling curve: temperature decreases from max to min
...
def calculate_enhanced_drying_rate_constant(SI, VPD, DAY, SM, SD, AR=0, wind_speed=5.0, crop_type="alfalfa"):
    """
    Enhanced drying rate calculation with additional factors
    ...
    # Wind speed factor (empirical relationship)
    wind_factor = 1 + (0.05 * wind_speed)  # 5% increase per m/s
    
    # Enhanced drying rate formula
    numerator = (SI * (1.0 + 9.03*AR) * wind_factor * crop_factor) + (43.8 * VPD)
    denominator = (61.4 * SM) + SD * (1.82 - 0.83 * DAY) * ((1.68 + 24.8 * AR)) + 1500
    
    drying_rate = numerator / denominator

    if SI > 300 and VPD > 1.0:  # Good drying weather
        drying_rate *= 1.8  # Increase rate by 80%
    
    # NEW: Higher maximum rate for initial drying
    return max(min(drying_rate, 0.8), 0.001)  # 80% per day max instead of 50%
...
def predict_moisture_content_enhanced(df, startdate, swath_density=450, starting_moisture=0.80, 
    ...
    """Enhanced moisture content prediction with better modeling"""
    try:
        ...
        for idx, row in df.iterrows():
            ...
            # Get parameters
            SI = max(row.get('peak_solarradiation', 300), 0)  # Ensure positive
            VPD = max(row.get('vapor_pressure_deficit', 1.0), 0)  # Ensure positive
            SM = row.get('soilmoisturevol01', 0.1) * 100 if not pd.isna(row.get('soilmoisturevol01', np.nan)) else 10
            wind_actual = row.get('windspeed', wind_speed)  # Use actual wind if available
            
            # Calculate drying rate
            k = calculate_enhanced_drying_rate_constant(
                SI, VPD, DAY, SM, swath_density, application_rate, wind_actual, crop_type
            )

            # Update moisture content using exponential decay
            current_moisture *= np.exp(-k)
            
            # Ensure moisture doesn't go below equilibrium moisture (about 6%)
            current_moisture = max(current_moisture, 0.06)
            
            moisture_contents.append(current_moisture)
            drying_rates.append(k)

            # Stop if moisture reaches equilibrium
...
def main():
    ...
    st.markdown("""
        ...
        <p style='text-align: center; font-size: 18px; color: #555; margin-bottom: 40px;'>
            Advanced Weather-Optimized Crop Management Tools
        </p>
    """, unsafe_allow_html=True)
        ...
        # Drying Calculation inputs
            ...
            with col1:
                crop_type = st.selectbox("Crop Type", 
                                       options=["alfalfa", "grass", "clover", "timothy", "fescue"],
                                       index=0 if st.session_state['crop_type'] == 'alfalfa' else 
                                             ["alfalfa", "grass", "clover", "timothy", "fescue"].index(st.session_state['crop_type']),
                                       help="Type of crop being dried")
            
            with col2:
                starting_moisture = st.slider("Starting Moisture (%)", 
                                            min_value=10, max_value=95, 
                                            value=int(st.session_state['starting_moisture'] * 100),
                                            help="Initial moisture content at harvest") / 100

            col1, col2 = st.columns(2)
            with col1:
                swath_density = st.number_input("Swath Density (g/m²)", 
                                              value=st.session_state['swath_density'], 
    ...
    if not (algorithm == "GDD Calculation" and 'calculate_button' in locals() and calculate_button) and \
       not (algorithm == "Drying Calculation" and 'drying_button' in locals() and drying_button):
        ...
        with col2:
            ...
            - **Factors**: Solar radiation, humidity, wind, crop type
